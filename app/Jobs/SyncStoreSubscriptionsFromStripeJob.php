...
class SyncStoreSubscriptionsFromStripeJob implements ShouldQueue
{
    ...
    {
        Log::info('Starting sync subscriptions from Stripe for store', [
        ...
        try {
            ...
            $syncAction = new SyncConnectedSubscriptionsFromStripe();
            $result = $syncAction($this->store, false); // Don't send notifications from job

            Log::info('Sync subscriptions from Stripe completed for store', [
                'store_id' => $this->store->id,
                'total' => $result['total'],
            ...
            Log::error('Sync subscriptions from Stripe job failed for store', [
                'store_id' => $this->store->id,
                'error' => $e->getMessage(),
