...
class SyncStoreCustomersFromStripeJob implements ShouldQueue
{
    ...
    {
        Log::info('Starting sync customers from Stripe for store', [
        ...
        try {
            ...
            $syncAction = new SyncConnectedCustomersFromStripe();
            $result = $syncAction($this->store, false); // Don't send notifications from job

            Log::info('Sync customers from Stripe completed for store', [
                'store_id' => $this->store->id,
                'total' => $result['total'],
            ...
            Log::error('Sync customers from Stripe job failed for store', [
                'store_id' => $this->store->id,
                'error' => $e->getMessage(),
