...
/// Syncs `clearDeferredResumeContext` from generated_code export into FlutterFlow POSitiv.
///
/// Source: [repo]/generated_code/lib/custom_code/actions/clear_deferred_resume_context.dart
Future<void> main(List<String> args) async {
  final options = _parseCliOptions(args);
  final scriptDir = File(Platform.script.toFilePath()).parent;
  final workspaceRoot = scriptDir.parent;
  final sourceFile = File(
    '${positivWorkspaceRoot()}/generated_code/lib/custom_code/actions/clear_deferred_resume_context.dart',
  );
    ...
    await flutterFlowAI(
      (app) {
        app.raw((project) {
          ...
          if (existing == null) {
            throw StateError(
              'Custom action clearDeferredResumeContext not found. '
          ...
          updateCustomAction(
            ...
            code: code,
            description:
                'Removes deferred resume prefs; optional FFAppState banner mirror off.',
          );
        });
      },
      ...
      commitMessage:
          ...
          'fix(pos): clearDeferredResumeContext — sync banner app state',
    );
  } catch (error) {
...
void _printUsage() {
  stdout.writeln('''
Update clearDeferredResumeContext from generated_code export into FlutterFlow POSitiv.

Usage (from positiv_flutterflow_ai/):
  dart run dsl/update_clear_deferred_resume_context.dart [options]
