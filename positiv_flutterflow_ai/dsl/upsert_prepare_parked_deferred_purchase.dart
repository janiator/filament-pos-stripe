...
/// Adds or updates `prepareParkedDeferredPurchase` from generated_code export into
/// FlutterFlow POSitiv (`pointofsale-xrlz5i`).
///
/// Source: [repo]/generated_code/lib/custom_code/actions/prepare_parked_deferred_purchase.dart
Future<void> main(List<String> args) async {
  final options = _parseCliOptions(args);
  final scriptDir = File(Platform.script.toFilePath()).parent;
  final workspaceRoot = scriptDir.parent;
  final sourceFile = File(
    '${positivWorkspaceRoot()}/generated_code/lib/custom_code/actions/prepare_parked_deferred_purchase.dart',
  );
    ...
    await flutterFlowAI(
      (app) {
        app.raw((project) {
          ...
          const description =
              'GET purchase, hydrate FFAppState cart + totals for parked deferred; '
              'persists resume prefs for pos banner; returns cartJson for completeDeferredPayment.';
      ...
      commitMessage:
          ...
          'feat(pos): prepareParkedDeferredPurchase (orders parked deferred)',
    );
  } catch (error) {
...
void _printUsage() {
  stdout.writeln('''
Add or update prepareParkedDeferredPurchase from generated_code export into FlutterFlow POSitiv.

Usage (from positiv_flutterflow_ai/):
  dart run dsl/upsert_prepare_parked_deferred_purchase.dart [options]
