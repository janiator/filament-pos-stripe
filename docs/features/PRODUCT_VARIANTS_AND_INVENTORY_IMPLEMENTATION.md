...
- **Deferred payments:** Stock is reduced when the deferred sale is created (same as paid sale). When completing with **no** `cart` payload, stock is unchanged at completion. When completing with a **final `cart`** (parked / edited order), the server applies an **inventory delta** between the original deferred lines and the final cart (extra sale movements or manual-adjust restores), idempotent per charge and variant.
