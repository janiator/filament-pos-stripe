...
- **Tripletex VAT types on Z (optional)** — `ledger.tripletex_vat_type_sales` (map of basis key, e.g. `"25"`, to Tripletex VAT type id) and `ledger.tripletex_vat_type_output_vat` for the output VAT posting. Omitted keys leave postings without `vatType` in Tripletex JSON. **If Tripletex rejects the voucher with HTTP 422**, set the correct Tripletex VAT type ids for each basis key you use (see Filament **Tripletex** ledger fields `ledger_tripletex_vat_sales_*`); many companies require `vatType` on turnover postings even when the Z-report has no separate output-VAT line.
