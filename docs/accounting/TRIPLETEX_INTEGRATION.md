...
`TripletexPeriodPreviewService` loops eligible sessions and payouts for a calendar range and reuses the same preview builders; compact mode strips raw `lines` to keep responses smaller while `rollup` merges debit/credit totals and `line_kinds` across the period. Filament’s **Preview period** runs this work in `BuildTripletexPeriodPreviewJob` (queued, 15‑minute timeout, unique per store while running) and persists progress plus the finished payload in `tripletex_integrations.period_preview_state` (JSON: `status` `processing` | `complete` | `failed`, optional `result`, optional `error`, `updated_at`) so web requests and queue workers always read the same store. The Tripletex settings page polls until the job finishes. For long months, run queue workers with a non-`sync` queue connection so the job is not forced into the web request.
