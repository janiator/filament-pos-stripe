...
| Payout voucher | `StoreStripePayout` saved as `paid` (when auto-sync on) or manual/API/Filament | `tripletex:payout:{store_id}:{stripe_payout_id}` | Bank vs Stripe clearing; fees from balance tx rows with same `stripe_payout_id` (optionally split into **application fee** vs **Stripe processing fee** when mirror `fee_details` exist and `settings.ledger.application_fee.debit_account_no` is set). Optional **external / web ticket** paired lines when `settings.ledger.external_ticket_sales` is enabled (see below). Per-store **`tripletex_integrations.skip_payout_bank_transfer`**: when true, the main **clearing-to-bank** pair (configured payout credit vs bank debit, often 1901 → 1920) is omitted for automatic sync, payout preview, Filament retry, and API payout/retry/historical calls that do not pass an explicit override; fees and external-ticket lines still post when present. |
...
- **VAT basis mapping (Z-report):** With `mapping_basis = vat`, configure **sales account numbers per standard rate** (0%, 15%, 25%) in Filament. When the Z-report snapshot includes **`sales_net_minor_by_vat_rate`** and **`vat_minor_by_vat_rate`** (computed from receipt/charge line items and product `vat_percent` when totals reconcile), Tripletex posts **separate net sales lines per rate** and **separate output-VAT lines per rate**; otherwise a single bucket uses `net_amount`, `vat_amount`, and `vat_rate` as before. **When those per-rate arrays are present**, the voucher builder uses them for sales credits and VAT types even if `mapping_basis` is payment method / vendor / category: sales are still split by rate (so `ledger.tripletex_vat_type_sales` keys `0` / `15` / `25` apply), while **sales account numbers** for that case come from the dominant payment-method mapping, `ledger.default_sales_account_no`, or the first mapping row—keep VAT-type overrides aligned with the same rates.
...
- **Tripletex VAT types on Z (optional)** — `ledger.tripletex_vat_type_sales` (map of basis key, e.g. `"25"`, to Tripletex VAT type id) and `ledger.tripletex_vat_type_output_vat` for the output VAT posting. When a key is **omitted**, `TripletexManualVoucherPayloadFactory` still sends `vatType.id` on each posting **from the resolved Tripletex account** (`GET /ledger/account` → `vatType.id`), so locked accounts validate without Filament values. Filled keys **override** that default for the matching line. **If Tripletex still rejects with HTTP 422**, check account/VAT setup in Tripletex or set overrides explicitly.
...
### HTTP 422 (`Validering feilet`)

Tripletex’s JSON body often includes `validationMessages` (sometimes nested under `value`). The app logs the full response and merges those messages into the sync failure text when possible. Typical Z-report causes: **missing or wrong Tripletex VAT type** on sales lines (`ledger.tripletex_vat_type_sales`), **posting date in a locked period**, **account number not in the Tripletex chart**, or **VAT type incompatible with the chosen account**. Use **Preview Tripletex voucher** with **Resolve Tripletex account IDs** enabled, then retry sync after fixing settings in Tripletex or Filament.
