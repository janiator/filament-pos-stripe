...
- Payment debits are gross per method (cash / card). Line descriptions use **Kasse {enhetsnavn}** (and **Vipps** suffix for Vipps). Sales/reskontro lines use **leverandørnavn** or **Salg {varegruppe}** — not repeated Z-report prefixes on each line. To mirror the accountant exactly (bank debited directly, no interim/fee/payout lines), set `payment_debits.card` to the **bank account** (e.g. 1920) and leave the **payment fee** and **payout** account pairs empty in PowerOffice settings. Configure fee/payout pairs only if you want the Stripe settlement modelled through an interim account instead.
