...
6. **Sync → Post directly to the ledger** — Leave **on** for immediate posting (`POST /Vouchers/ManualJournals`, [Voucher Posting](https://developer.poweroffice.net/workflows/endpoints/voucher-workflows/voucher-posting)). Turn **off** to send Z-report vouchers as journal-entry drafts in Go for accountant review (`POST /JournalEntryVouchers/ManualJournals`, [Journal Entry Voucher](https://developer.poweroffice.net/endpoints/voucher-workflows/journalentryvoucher)). PDF documentation is still attached when Go returns a voucher Id.
