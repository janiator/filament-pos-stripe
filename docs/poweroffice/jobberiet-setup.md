...
- Commission vendors: full vendor turnover on reskontro (gross `amount` from Z-report); all provision is aggregated to **one** 3023 line (`Salg Stuttreist`), not one 3023 line per vendor. Reskontro is credited net (`amount − provision`) so the voucher stays balanced.
- Payment debits are gross per method (cash / card). Line descriptions use **Kasse {enhetsnavn}** (and **Vipps** suffix for Vipps). Sales/reskontro lines use **leverandørnavn** or **Salg {varegruppe}** — not repeated Z-report prefixes on each line. To mirror the accountant exactly (bank debited directly, no interim payout lines), set `payment_debits.card` to the **bank account** (e.g. 1920) and turn off **Post Stripe fees and bank payout on Z-report voucher** in Ledger routing (Stripe fees still post to the payment fee account pair below; only the bank payout pair is skipped). Configure fee/payout pairs only if you want the full Stripe settlement modelled through an interim account instead.
...
6. **Sync → Post directly to the ledger** — Leave **on** for immediate posting (`POST /Vouchers/ManualJournals`, [Voucher Posting](https://developer.poweroffice.net/workflows/endpoints/voucher-workflows/voucher-posting)). Turn **off** to send Z-report vouchers as journal-entry drafts in Go for accountant review (`POST /JournalEntryVouchers/ManualJournals`, [Journal Entry Voucher](https://developer.poweroffice.net/endpoints/voucher-workflows/journalentryvoucher)). Journal-entry payloads use `ManualVoucherLines` with separate debit/credit account ids (not the direct-posting `VoucherLines` shape). Z-report PDFs attach via `PUT /VoucherDocumentation` (direct) or `POST /JournalEntryVouchers/{id}/VoucherPages` (journal-entry draft).
