...
class PdmInternalLibrary extends StatefulWidget {
  const PdmInternalLibrary({
    super.key,
    this.width,
    this.height,
  ...
  @override
  State<PdmInternalLibrary> createState() =>
      _PdmInternalLibraryState();
}

class _PdmInternalLibraryState extends State<PdmInternalLibrary> {
  ...
  final TextEditingController _deviceIdController =
      TextEditingController(text: 'local_printer');
  ...
  Future<void> _scanNetwork() async {
    ...
    try {
      ...
      if (kIsWeb) {
        throw Exception(
            'Network scanning is not available on web. Please use manual entry.');
      }
      ...
      if (mounted && detected.isEmpty) {
        setState(() {
          _errorMessage =
              'No printers found. Try manual entry or ensure printers are on the same network.';
        });
      }
    } catch (e) {
      if (mounted) {
        ...
        for (final interface in interfaces) {
          ...
          for (final addr in interface.addresses) {
            if (addr.type == InternetAddressType.IPv4 &&
              ...
              debugPrint(
                  'Found local IP via NetworkInterface: ${addr.address} (interface: ${interface.name})');
              return addr.address;
            }
          }
        }
        ...
        for (final interface in interfaces) {
          for (final addr in interface.addresses) {
            if (addr.type == InternetAddressType.IPv4 &&
                _isPrivateIp(addr.address)) {
              debugPrint(
                  'Found local IP via NetworkInterface (fallback): ${addr.address} (interface: ${interface.name})');
              return addr.address;
            }
          }
        }
  ...
  Future<Map<String, dynamic>?> _getEpsonPrinterInfo(
      String ip, bool useHttps) async {
    try {
      final protocol = useHttps ? 'https' : 'http';
      final url = Uri.parse(
          '$protocol://$ip/cgi-bin/epos/service.cgi?devid=local_printer&timeout=5000');
...
<epos-print xmlns="http://www.epson-pos.com/schemas/2011/03/epos-print">
  <request>
    <status />
      ...
      if (response.statusCode >= 200 && response.statusCode < 500) {
        ...
        debugPrint(
            '$ip - Response body (first 500 chars): ${body.length > 500 ? body.substring(0, 500) : body}');
        ...
        final hasEposNamespace =
            bodyLower.contains('http://www.epson-pos.com/schemas');
        ...
        final hasEpsonInHtml = isHtmlPage &&
            (bodyLower.contains('epson') ||
        ...
        final hasEpsonTitle =
            bodyLower.contains('<title') && bodyLower.contains('epson');

        debugPrint(
            '$ip - Checks: ePOSResponse=$hasEposResponse, namespace=$hasEposNamespace, header=$hasEpsonHeader, html=$isHtmlPage, epsonInHtml=$hasEpsonInHtml, epsonTitle=$hasEpsonTitle');
        ...
        if (hasEposResponse ||
          ...
          final modelMatch =
              RegExp(r'<model[^>]*>([^<]+)</model>', caseSensitive: false)
                  .firstMatch(body);
          if (modelMatch != null) {
          ...
          if (model == null || model.isEmpty) {
            final deviceMatch =
                RegExp(r'<device[^>]*model="([^"]+)"', caseSensitive: false)
                    .firstMatch(body);
            if (deviceMatch != null) {
          ...
          if (model == null || model.isEmpty) {
            ...
            if (bodyLower.contains('tm-m30iii-h') ||
                bodyLower.contains('tm-m30-iii-h')) {
              model = 'TM-m30III-H';
              printerName = 'Epson TM-m30III-H';
            } else if (bodyLower.contains('m30') ||
                bodyLower.contains('tm-m30') ||
                bodyLower.contains('tm30')) {
              model = 'TM-m30III';
              printerName = 'Epson TM-m30III';
            } else if (bodyLower.contains('tm-')) {
              final tmMatch = RegExp(r'tm-?([a-z0-9-]+)', caseSensitive: false)
          ...
          return {
            ...
            'use_https': useHttps,
            'device_id':
                'local_printer', // Always use local_printer for ePOS-Print
            'detected': true,
          };
        } else {
    ...
    try {
      ...
      for (final serviceType in serviceTypes) {
        ...
        try {
          ...
          // Query for services
          await for (final PtrResourceRecord ptr in mdns
            ...
            try {
              ...
              await for (final SrvResourceRecord srv in mdns
                ...
                // Get the IP address
                await for (final IPAddressResourceRecord ip in mdns
                  ...
                  final serviceName =
                      ptr.domainName.replaceAll('.$serviceType', '');

                  debugPrint('Found mDNS service: $serviceName at $ipAddress');

                  // Check if it's an Epson (or Epson-compatible) printer by name, or any _printer._tcp service
                  final nameLower = serviceName.toLowerCase();
                  final isEpsonByName = nameLower.contains('epson') ||
                      nameLower.contains('tm-') ||
                      nameLower.contains('merano');
                  final isPrinterServiceType =
                      serviceType == '_printer._tcp.local';
                  final shouldTryVerify = isEpsonByName || isPrinterServiceType;
                  ...
                  if (shouldTryVerify) {
                    ...
                    if (nameLower.contains('tm-')) {
                      final modelMatch =
                          RegExp(r'tm-?([a-z0-9-]+)', caseSensitive: false)
                              .firstMatch(nameLower);
                      if (modelMatch != null) {
                        model =
                            'TM-${modelMatch.group(1)?.toUpperCase() ?? ""}';
                        printerName = 'Epson $model';
                      }
                    }
                    ...
                    final printerInfo =
                        await _getEpsonPrinterInfo(ipAddress, false);
                    if (printerInfo != null) {
                      ...
                      printerInfo['bonjour_name'] =
                          serviceName; // Store Bonjour name for unique identification
                      discovered.add(printerInfo);
                      debugPrint(
                          '✓ Discovered Epson printer via Bonjour: ${printerInfo['name']} at $ipAddress (Bonjour: $serviceName)');
                    } else {
                      ...
                      discovered.add({
                        ...
                        'use_https': false,
                        'device_id':
                            'local_printer', // Always use local_printer
                      ...
                      debugPrint(
                          '✓ Discovered Epson printer via Bonjour (no ePOS response): $printerName at $ipAddress (Bonjour: $serviceName)');
                    }
                  }
                }
              }
  ...
  Future<Map<String, dynamic>?> _checkEpsonWebInterface(String ip) async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        debugPrint(
            '$ip - Web interface check: header=$hasEpsonHeader, body=$hasEpsonInBody, title=$hasEpsonTitle');
  ...
  Map<String, dynamic>? _findExistingPrinter(
      Map<String, dynamic> detectedPrinter) {
    ...
    if (printerName.isNotEmpty &&
      ...
      for (final existing in _existingPrinters) {
        ...
        if (existingName.toLowerCase() == printerName.toLowerCase() &&
          ...
          debugPrint(
              'Found existing printer by name + model: $printerName ($printerModel)');
          return existing;
        }
      }
    }
  ...
  void _selectDetectedPrinter(Map<String, dynamic> printer) async {
    ...
    if (existingPrinter != null && existingPrinter['id'] != null) {
      ...
      if (existingIp.toLowerCase() != newIp.toLowerCase() && newIp.isNotEmpty) {
        ...
        debugPrint(
            'IP changed for printer ${existingPrinter['id']}: $existingIp -> $newIp');
    ...
    setState(() {
      ...
      _nameController.text =
          printer['name'] as String? ?? 'Printer ${printer['ip_address']}';
  ...
  Future<void> _checkIfPrinterIsDefault(int printerId) async {
    ...
    try {
      final uri = Uri.parse(
          '${widget.apiBaseUrl}/api/pos-devices/${widget.currentPosDeviceId}');
      final response = await http.get(
        uri,
        headers: {
          'Authorization': 'Bearer ${widget.authToken}',
    ...
    try {
      ...
      final printerData = {
        ...
        if (widget.currentPosDeviceId != null)
          'pos_device_id': widget.currentPosDeviceId,
      };
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        ...
        debugPrint(
            'Printer ${isUpdate ? 'updated' : 'registered'} successfully. Printer ID: $printerId');
        ...
        if (_setAsDefault &&
            widget.currentPosDeviceId != null &&
            printerId != null) {
          debugPrint(
              'Setting printer $printerId as default for POS device ${widget.currentPosDeviceId}...');
          await _updatePosDeviceDefaultPrinter(
              widget.currentPosDeviceId!, printerId);
        } else if (!_setAsDefault &&
            widget.currentPosDeviceId != null &&
            printerId != null) {
          // If switch is off and this printer is currently the default, clear it
          debugPrint(
              'Not setting as default. Checking if we need to clear existing default...');
          await _checkAndClearDefaultPrinter(
              widget.currentPosDeviceId!, printerId);
        } else {
          if (widget.currentPosDeviceId == null) {
            debugPrint(
                '⚠ No current POS device ID provided - cannot set default printer');
          }
          if (printerId == null) {
  ...
  Future<void> _checkAndClearDefaultPrinter(
      int posDeviceId, int printerId) async {
    try {
      final uri =
          Uri.parse('${widget.apiBaseUrl}/api/pos-devices/$posDeviceId');
      final response = await http.get(
        uri,
        headers: {
          'Authorization': 'Bearer ${widget.authToken}',
  ...
  Future<void> _updatePosDeviceDefaultPrinter(
      int posDeviceId, int? printerId) async {
    try {
      final uri =
          Uri.parse('${widget.apiBaseUrl}/api/pos-devices/$posDeviceId');
      ...
      debugPrint(
          'Updating POS device $posDeviceId default printer to $printerId...');
      ...
      if (response.statusCode == 200) {
        ...
        if (printerId != null) {
          debugPrint(
              '✓ Updated POS device $posDeviceId default printer to $printerId');
          debugPrint('  Response default_printer_id: $updatedDefaultPrinterId');

          if (updatedDefaultPrinterId != printerId) {
            debugPrint(
                '⚠ WARNING: Default printer ID mismatch! Expected $printerId, got $updatedDefaultPrinterId');
          }
        } else {
          ...
          try {
            ...
            if (deviceMap['device_metadata'] != null &&
              ...
              deviceMap['device_metadata'] =
                  jsonEncode(deviceMap['device_metadata']);
            }
            ...
            debugPrint(
                '✓ Updated activePosDevice app state with new default printer settings');
          } catch (e) {
        ...
        debugPrint(
            '⚠ Failed to update POS device default printer: ${response.statusCode}');
        ...
        if (mounted) {
          ...
          final errorMessage =
              errorData?['message'] ?? 'Failed to set default printer';
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content:
                Text('Warning: Failed to set default printer: ${e.toString()}'),
            backgroundColor: Colors.orange,
            duration: const Duration(seconds: 4),
          ),
        );
                ...
                : RefreshIndicator(
                    ...
                    child: ListView.separated(
                      ...
                      itemBuilder: (context, index) {
                        ...
                        final posDevice =
                            printer['pos_device'] as Map<String, dynamic>?;
  ...
  Widget _buildPrinterForm() {
    ...
    debugPrint(
        '_buildPrinterForm: currentPosDeviceId = ${widget.currentPosDeviceId}');
          ...
          return SizedBox(
            ...
            child: Column(
              ...
              children: [
                ...
                // IP Address and Port in 2-col row
                Row(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Expanded(
                ...
                // Device ID, Printer Type, Paper Width in 3-col row
                Row(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Expanded(
                    ...
                    Expanded(
                      child: Padding(
                        ...
                        child: DropdownButtonFormField<String>(
                          ...
                          items: const [
                            DropdownMenuItem(
                                value: 'epson', child: Text('Epson')),
                            DropdownMenuItem(
                                value: 'star', child: Text('Star')),
                            DropdownMenuItem(
                ...
                SwitchListTile(
                  ...
                  subtitle: widget.currentPosDeviceId != null
                      ? const Text(
                          'Make this printer the default receipt printer for the current POS device')
                      : const Text(
                          'No current POS device - cannot set as default'),
                  value: _setAsDefault,
