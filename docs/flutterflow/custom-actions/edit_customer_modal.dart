...
// FlutterFlow Custom Action: Edit Customer Modal
//
// This action shows a modal dialog for editing an existing customer.
// It handles the form UI and API calls to update customers.
//
// IMPORTANT: FlutterFlow requires Future<dynamic> as return type

Future<dynamic> editCustomerModal(
  ...
  try {
    ...
    // Check if customer has a valid ID
    bool isEdit = false;
    CustomersStruct? customerData;
    ...
    try {
      ...
      if (customerId != null && customerId > 0) {
        ...
        return {
          'success': false,
          'message': 'Invalid customer: customer ID is required for editing',
        };
      }
    } catch (e) {
      return {
        'success': false,
        'message': 'Invalid customer data: ${e.toString()}',
      };
    }
    ...
    final line1Controller = TextEditingController(
        text: customerData?.customerAddress?.line1 ?? '');
    final line2Controller = TextEditingController(
        text: customerData?.customerAddress?.line2 ?? '');
    final cityController = TextEditingController(
        text: customerData?.customerAddress?.city ?? '');
    final stateController = TextEditingController(
        text: customerData?.customerAddress?.state ?? '');
    final postalCodeController = TextEditingController(
        text: customerData?.customerAddress?.postalCode ?? '');
    final countryController = TextEditingController(
        text: customerData?.customerAddress?.country ?? 'NO');
