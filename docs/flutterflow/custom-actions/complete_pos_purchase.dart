...
Future<dynamic> completePosPurchase(
  ...
  try {
    ...
    if (additionalMetadataJson != null && additionalMetadataJson.isNotEmpty) {
      try {
        additionalMetadata = jsonDecode(additionalMetadataJson) as Map<String, dynamic>;
      } catch (e) {
    ...
    if (isSplitPayment && splitPaymentsJson != null && splitPaymentsJson.isNotEmpty) {
      try {
        final List<dynamic> parsed = jsonDecode(splitPaymentsJson) as List<dynamic>;
        splitPayments = parsed.map((item) => item as Map<String, dynamic>).toList();
      } catch (e) {
        return {
          'success': false,
    ...
    final totalDiscounts = cart.cartTotalDiscount; // in øre (includes both item and cart discounts)
