...
// - CartDataStruct with CartTypes.product
// - ProductStruct (already a struct, no conversion needed)
// - ShoppingCartStruct with cartData, employee, customer
//
// Function signature:
Future addItemToCart(ProductStruct product, int quantity, [VariantsStruct? variant]) async {
  ...
  // Note: If you need to distinguish by variant, you'll need to add variant tracking
  ...
  if (existingIndex >= 0) {
    ...
    final newCartItem = CartDataStruct(
      ...
      quantity: quantity,
      product: product,
      customer: null, // Or set to currentCart.customer if you want to associate customer with item
