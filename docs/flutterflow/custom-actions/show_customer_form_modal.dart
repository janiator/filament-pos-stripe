...
// FlutterFlow Custom Action: Show Customer Form Modal (Internal Helper)
//
// This is an internal helper function used by createCustomerModal and editCustomerModal.
// Do not use this directly in FlutterFlow - use createCustomerModal or editCustomerModal instead.
//
// IMPORTANT: FlutterFlow requires Future<dynamic> as return type

Future<dynamic> showCustomerFormModal(
  BuildContext context,
  String apiBaseUrl,
  String authToken,
  CustomersStruct existingCustomer,
) async {
  try {
    // Validate inputs
    ...
    // Check if customer has a valid ID (not 0 or null)
    // If ID is 0 or null, treat as new customer
    bool isEdit = false;
    ...
    try {
      ...
      if (customerId != null && customerId > 0) {
        ...
        // Empty/default customer struct - treat as new customer
        isEdit = false;
        customer = null;
      }
    } catch (e) {
      // If we can't access the ID, treat as new customer
      isEdit = false;
      customer = null;
    }
    ...
    final line1Controller = TextEditingController(
        text: customer?.customerAddress?.line1 ?? '');
    final line2Controller = TextEditingController(
        text: customer?.customerAddress?.line2 ?? '');
    final cityController = TextEditingController(
        text: customer?.customerAddress?.city ?? '');
    final stateController = TextEditingController(
        text: customer?.customerAddress?.state ?? '');
    final postalCodeController = TextEditingController(
        text: customer?.customerAddress?.postalCode ?? '');
    final countryController = TextEditingController(
        text: customer?.customerAddress?.country ?? 'NO');
