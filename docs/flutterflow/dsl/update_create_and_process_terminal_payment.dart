...
/// Syncs `createAndProcessTerminalPayment` custom action into FlutterFlow
/// (POSitiv `pointofsale-xrlz5i`).
///
/// Source of truth: `docs/flutterflow/custom-actions/create_and_process_terminal_payment.dart`.
Future<void> main(List<String> args) async {
  final options = _parseCliOptions(args);
  final scriptDir = File(Platform.script.toFilePath()).parent;
  final repoRoot = scriptDir.parent.parent;
  final sourceFile = File(
    '${repoRoot.path}/docs/flutterflow/custom-actions/create_and_process_terminal_payment.dart',
  );
  ...
  try {
    await flutterFlowAI(
      (app) {
        app.raw((project) {
          ...
          updateCustomAction(
            ...
            code: code,
            description:
                'Create and process POS terminal payments with Stripe defaults and optional Verifone provider parameters.',
          );
        });
      },
      ...
      commitMessage:
          ...
          'fix(pos): keep terminal payment action backwards compatible',
    );
  } catch (error) {
...
void _printUsage() {
  stdout.writeln('''
Push createAndProcessTerminalPayment from docs/flutterflow/custom-actions into FlutterFlow.
...
copy scripts from docs/flutterflow/dsl/ after `flutterflow ai init` (never name the workspace `flutterflow_ai`).
