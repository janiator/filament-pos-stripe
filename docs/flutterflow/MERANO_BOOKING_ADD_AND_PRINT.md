...
**Automatic (recommended):** The custom action `completePosPurchase` already confirms Merano bookings after a successful purchase. It uses the charge identifier from the API response (`stripe_charge_id` for Stripe, or `transaction_code` / charge `id` for cash) so confirm is called for both Stripe and cash payments. It groups cart lines by `merano_booking_id`, sums the **amount actually paid** per booking (after per-line discounts), and calls `confirmMeranoBookingAfterPayment` once per booking. The amount sent must be after discounts: use `(unitPrice * qty) - ((cartItemDiscountAmount ?? 0) * qty)` per line (same convention as `update_cart_totals`). No extra FlutterFlow steps needed if you use `completePosPurchase`.

**Manual:** If you complete payment outside `completePosPurchase`, for each distinct `merano_booking_id` in the cart (from `item.cartItemMetadata.notes`), sum the line amounts **after discount** for that booking and call `confirmMeranoBookingAfterPayment(bookingId, totalAmountPaidOre, posChargeId, apiBaseUrl, authToken, ...)`. See the comment block in `confirm_merano_booking_after_payment.dart` for parameters.

**Fix in FlutterFlow (completePosPurchase):** If your `completePosPurchase` custom action still sends the pre-discount amount to Merano, update the loop that builds `amountByBookingId`. Replace the per-line amount calculation from `final amountPaidOre = (unitPrice * qty).round();` with: `final discountPerUnit = item.cartItemDiscountAmount ?? 0;` and `final amountPaidOre = ((unitPrice - discountPerUnit) * qty).round();`. Then add each line's `amountPaidOre` to the booking's sum as you do now. The backend does not recalculate; it forwards whatever amount the client sends.
